    ...
    <>
                  ...
                  {adminUsers.map(item => (
                    <article className="user-card" key={item.id}>
                      ...
                      <div className="user-badges">
                        ...
                        <Badge tone="neutral">{departmentLabel(item.role, item.department)}</Badge>
                      </div>
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="إضافة أدمن جديد" subtitle="إنشاء حساب إداري داخلي للنظام" size="lg">
        <form className="form-grid" autoComplete="off" onSubmit={createUser}>
          ...
          <Field label="القسم">
            <select value={userForm.role === 'admin' ? 'admin-department' : userForm.department} disabled>
      ...
      <Modal open={editOpen} onClose={() => setEditOpen(false)} title="تعديل المستخدم" subtitle={selectedUser ? `تحديث حساب ${selectedUser.name}` : ''} size="lg">
        <form className="form-grid" onSubmit={updateUser}>
          ...
          <Field label="الدور">
            <select value={userForm.role} onChange={event => setUserForm({ ...userForm, role: event.target.value, department: event.target.value === 'admin' ? 'Management' : userForm.department === 'Management' ? 'Consultancy' : userForm.department })}>
              <option value="admin">مسؤول النظام</option>
              <option value="management">الإدارة</option>
          ...
          <Field label="القسم">
            {userForm.role === 'admin' ? (
              <select value="admin-department" disabled>
                <option value="admin-department">مسؤول</option>
              </select>
            ) : (
